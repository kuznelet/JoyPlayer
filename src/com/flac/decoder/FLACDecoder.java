...
import com.audioplayer.GraphAndSound.FormatParser;
import com.audioplayer.GraphAndSound.GUISynthesiszer;

public final class FLACDecoder extends Decoder {
	...
	private static long totalSamplesInStream;
	...
	public final static Decoder createInstance(Path songPath) {
		decoder = null;
		try {
			decoder = new FLACDecoder(songPath);
	...
	// Constructor
	private FLACDecoder(Path songPath) throws IOException,
	...
	public static Object[] readNextBlock(boolean verifyCRC8)
		...
		if (verifyCRC8) {
			...
			if ((rawHeader.get(1) & 2) != 0)
				throw decoder.new BadFileFormatException("Reserved bit");
			...
			case 1:
				if (bitsPerSample != 8)
					throw decoder.new BadFileFormatException(
			...
			if (blockSizeCode == 1)
				...
				throw decoder.new BadFileFormatException("Reserved block size");
	...
	private static void decodeSubframe(int sampleDepth, long[] result)
			throws BadFileFormatException, IOException {
		if (input.readBit(1) != 0)
			throw decoder.new BadFileFormatException("Invalid padding bit");
		...
		if (type == 0) // Constant coding
			...
			throw decoder.new BadFileFormatException("Invalid predicton order.");
		for (int i = 0; i < result.length; i++)
			result[i] <<= shift;
	}
	...
	final protected boolean sampleProcessor(double request) throws IOException,
			BadFileFormatException, LineUnavailableException,
			InterruptedException {
		// Invoking two boolean fields to avoid frequent and unnecessary
		// evaluation of endWith()
		long[][] samples = null;
		if (request <= 1) {
			long samplePos = Math.round(this.getTotalSamplesInStream()
					* request);
			samples = ((FLACDecoder) Decoder.getInstance())
			...
			if (this.getCurrentStream() == null) {
				Logger.getGlobal().log(Level.INFO,
						"flac input stream is null, thread is quitting.");
				return false;
			}
			Object[] temp = FLACDecoder.readNextBlock(false);
			if (temp != null) {
				samples = (long[][]) temp[0];
			}
		}
		...
		// little-endian
		sampleInBytes = new byte[samples.length * samples[0].length
				* this.getBitsPerSample() / 8];
		for (int i = 0, k = 0; i < samples[0].length; i++) {
			for (int ch = 0; ch < samples.length; ch++) {
				long cache = samples[ch][i];
				for (int j = 0; j < this.getBitsPerSample() / 8; j++, k++)
					sampleInBytes[k] = (byte) (cache >>> (j << 3));
			}
		}
	 ...
	 * @throws IOException
	 */
	final protected double getCurrentPosition() throws IOException {
		return GUISynthesiszer.getCurrentTimeInMicros()
				* GUISynthesiszer.getOneInMillion()
				* FormatParser.getSampleRateInHz()
